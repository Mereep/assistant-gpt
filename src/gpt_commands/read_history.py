...
class ReadConversationHistory(ICommand):
    ...
    def description(cls) -> str:
        return f'reads / retrieves an old message of the conversation by its index. Only use this if you don\'t ' \
               f'have the information you need and you are sure we talked about this' \

    @classmethod
    def arguments(cls) -> list[CommandArgument]:
        return [
            CommandArgument(name='index',
                            type=int,
    ...
    def execute(self, chat_context: ChatContext, **args) -> str:
        ...
        if index >= len(chat_context.message_history):
            return "Index out of range. As of now, we only have {} messages in the message history." \
                .format(len(chat_context.message_history))

        message = chat_context.message_history[index]
        r = "Message at index #{}:\n".format(index)
        if isinstance(message, UserMessage):
            return r+f"User message from user `{message.user}`: \n{message.user_response}\n " \
