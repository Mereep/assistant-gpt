...
class NewsApiCommand(ICommand):
    ...
    def arguments(cls) -> list[CommandArgument]:
        return [
            CommandArgument(name='query', type=str, help='the query to search for',
                            required=True),
            CommandArgument(name='page', type=int, help='The page to return defaults to 1 if not given.',
                            required=False),
            CommandArgument(name='language', type=str, help='limit the search to a specific language like '
    ...
    def execute(self, chat_context: ChatContext, **args) -> str:
        ...
        for article in news_articles:
            article_str += f"- {article.published_at.strftime('%Y-%m-%d')}: " \
    ...
    if response.status_code == 200:
        ...
        news_articles = [
            NewsArticle(
                ...
                published_at=datetime.datetime.fromisoformat(article['publishedAt'][:-1]),
                url=article['url'],
        ...
        raise Exception(f"Couldn't fetch news data due to: {response.status_code} {response.text}")
