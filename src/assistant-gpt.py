...
def initiate_conversation(
    ...
    if available_conversation_ids:
        try:
            ...
            tell_human(
                _("Operation cancelled. Continuing with creating a new conversation."),
                app_settings=app_settings,
            )
            logger.error(
                f"Error while asking user if he wants to load a conversation due to {e}."
        ...
        if load in (_("yes"), _("y")):
            ...
            try:
                conversation_id = ask_human(
                    _("Which conversation do you want to continue? "),
                ...
                tell_human(
                    _(
                        "Operation cancelled. Continuing with creating a new conversation."
                    ),
                    app_settings=app_settings,
                )
                logger.error(
                    f"Error while asking user which conversation to load due to {e}."
                    f"Assuming user wants to start a new conversation."
                )
                conversation_id = None
            ...
            try:
                ...
                logger.error(
                    f"Error while loading conversation {conversation_id} due to {e}."
                )
                tell_human(
                    _(
                        "Error while loading conversation {conv}. Starting a new one."
            ...
            tell_human(
                _("Great, let's begin something new"),
                app_settings=app_settings,
    ...
    if not conversation:
        ...
        ai_name = "assistant"
        conversation_id = ask_human(
            _("How do we want to call this conversation? (to restore it later)"),
            style=typewrite_style,
            app_settings=app_settings,
        )
        ...
        conversation = ChatContext(
            ...
            key_storage_backend=load_key_storage_backend(
                app_settings=app_settings, conversation_id=conversation_id
            ),
...
def run_loop(conversation: ChatContext, logger: logging.Logger):
    ...
    while True:
        if len(conversation.message_history) == 0:
            user_turn = False
            tell_human(
                _("I will start a new conversation."),
                app_settings=conversation.settings,
            )
        else:
        ...
        if not user_turn:
            ...
            tell_human(
                _(
                    "Transmitting query with {n_tokens} tokens".format(
            ...
            try:
                ...
                tell_human(
                    _(
                        "Received a response with {n_tokens} tokens".format(
                            n_tokens=count_tokens(
                                response,
                                model=conversation.settings.model,
                                logger=logger,
                ...
                tell_human(
                    _("Error while talking to AI. Please try again."),
                    app_settings=conversation.settings,
                )
            ...
            if not response:
                ...
                parsed = try_parse_response(
                    response=response, ctx=conversation, logger=logger
                )
            ...
            present_bot_response_command(
                bot_response=bot_response, ctx=conversation, logger=logger
            )
            if bot_response.command == "gpt_response_error":
                tell_human(
                    _(
                        "Error while talking to AI. "
                ...
                try:
                    ...
                    response = (
                        f"Argument: {e.argument} has wrong type for command "
                ...
                tell_human(
                    _(
                        "----BEGIN RESULT OF COMMAND `{command}`----:\n"
                        "{response}\n"
                        "----END RESULT OF COMMAND `{command}`----"
                    ).format(response=response, command=bot_response.command),
                    app_settings=app_settings,
                )
